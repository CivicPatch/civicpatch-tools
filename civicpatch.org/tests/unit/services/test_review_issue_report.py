...
async def test_report_review_issue_files_github_issue_and_records_db_row():
    ...
    with review_p, github_p as mock_create_issue, db_p as mock_db_insert:
        result = await report_review_issue(REQUEST_ID, "Something looks wrong.", "user-1", "Jane Reviewer")
    ...
    assert "state=ca&request_id=req-1" in kwargs["body"]
    ...
    assert db_kwargs["request_id"] == REQUEST_ID
...
async def test_report_review_issue_omits_pr_line_when_no_pr_url():
    ...
    with review_p, github_p as mock_create_issue, db_p:
        await report_review_issue(REQUEST_ID, "Something looks wrong.", "user-1", "Jane Reviewer")
...
async def test_report_review_issue_raises_when_review_not_found():
    with patch(
        ...
        with pytest.raises(ReviewNotFoundError):
            await report_review_issue(REQUEST_ID, "desc", "user-1", "Jane Reviewer")
