...
def _asserted(field: str, accept=(), reject=()):
    return {field: {ClaimKind.ACCEPT: list(accept), ClaimKind.REJECT: list(reject)}}


@pytest.mark.unit
def test_a_person_nobody_has_looked_at_is_unchanged():
    person = {"name": "Jane Doe", "phones": ["(555) 0001"]}
    assert with_claimed_values(person, {}) == person
...
def test_a_rejected_number_stays_gone_however_often_it_is_scraped():
    ...
    claimed = _asserted("phones", reject=["(555) 0001"])
    assert with_claimed_values(person, claimed)["phones"] == ["(555) 0002"]
...
def test_a_number_nobody_rejected_still_arrives():
    ...
    claimed = _asserted("phones", reject=["(555) 0001"])
    assert with_claimed_values(person, claimed)["phones"] == ["(555) 9999"]


@pytest.mark.unit
def test_an_accepted_value_the_scrape_never_found_is_pinned():
    person = {"phones": []}
    claimed = _asserted("phones", accept=["(555) 8888"])
    assert with_claimed_values(person, claimed)["phones"] == ["(555) 8888"]
...
def test_scraped_order_comes_first_and_nothing_is_duplicated():
    ...
    person = {"phones": ["(555) 0001", "(555) 0002"]}
    claimed = _asserted("phones", accept=["(555) 0002", "(555) 8888"])
    assert with_claimed_values(person, claimed)["phones"] == [
        "(555) 0001",
        "(555) 0002",
        "(555) 8888",
    ]
...
def test_rejecting_beats_accepting_the_same_value():
    ...
    claimed = _asserted("phones", accept=["(555) 0001"], reject=["(555) 0001"])
    assert with_claimed_values(person, claimed)["phones"] == []
...
def test_a_scalar_is_replaced_not_unioned():
    ...
    claimed = _asserted("name", accept=["Jane Smith"])
    assert with_claimed_values(person, claimed)["name"] == "Jane Smith"


@pytest.mark.unit
def test_a_rejected_scalar_with_no_replacement_is_emptied():
    person = {"image": "https://x.gov/wrong.png"}
    claimed = _asserted("image", reject=["https://x.gov/wrong.png"])
    assert with_claimed_values(person, claimed)["image"] is None
...
def test_a_field_nobody_can_edit_is_left_alone():
    ...
    claimed = _asserted("cdn_image", accept=["https://cdn/other.png"])
    assert with_claimed_values(person, claimed)["cdn_image"] == "https://cdn/x.png"
...
def test_nothing_is_hidden_when_no_assertion_moved_a_value():
    ...
    person = {"name": "Jane Doe", "phones": ["(555) 0001"]}
    claimed = _asserted("phones", accept=["(555) 0001"])

    assert source_values_overridden(person, claimed) == {}


@pytest.mark.unit
def test_an_accept_that_replaced_the_scraped_value_discloses_it():
    person = {"name": "Jane Doe", "phones": ["(555) 0001"]}
    claimed = _asserted("phones", accept=["(555) 9999"])

    assert source_values_overridden(person, claimed) == {"phones": ["(555) 0001"]}
...
def test_a_field_outside_the_editable_set_is_never_disclosed():
    """The same guard `with_claimed_values` applies before overlaying: a field it will not
    ...
    not one, because it *is* editable. `NOT_CLAIMABLE` is a separate guard on the edit path."""
    person = {"memberships": [{"post_id": "post-1"}]}
    claimed = _asserted("memberships", accept=[{"post_id": "post-2"}])
