...
def test_extracts_the_key_from_an_artifacts_url():
    assert artifacts_key(_ARTIFACTS_URL) == _ARTIFACTS_KEY
...
def test_ignores_a_photo_hosted_elsewhere():
    ...
    assert artifacts_key("https://seattle.gov/img/jane.jpg") is None


@pytest.mark.unit
def test_ignores_an_already_promoted_url():
    """Publishing twice must not try to re-promote, or the second copy would 404."""
    assert artifacts_key("https://cdn.civicpatch.org/open-data/wa/local/x/images/j.jpg") is None
