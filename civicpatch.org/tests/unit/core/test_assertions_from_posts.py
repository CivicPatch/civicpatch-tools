"""`assertions_from_posts`: the posts a person should hold, as claims.
...
from core.people_edits import POSTS_FIELD, assertions_from_posts
from schemas.assertions import AssertionKind, DefaultNote, EntityType
...
def test_an_unchanged_set_files_nothing():
    ...
    assert assertions_from_posts("p1", [MAYOR], [MAYOR]) == []


@pytest.mark.unit
def test_a_new_post_is_an_accept():
    claims = assertions_from_posts("p1", [], [MAYOR])

    assert _pairs(claims) == [(AssertionKind.ACCEPT, MAYOR)]
...
def test_a_dropped_post_is_a_reject():
    assert _pairs(assertions_from_posts("p1", [MAYOR], [])) == [
        (AssertionKind.REJECT, MAYOR)
    ]
...
def test_a_move_is_both_halves():
    ...
    assert _pairs(assertions_from_posts("p1", [MAYOR], [CLERK])) == [
        (AssertionKind.ACCEPT, CLERK),
        (AssertionKind.REJECT, MAYOR),
    ]
...
def test_removing_a_person_rejects_every_post_they_hold():
    ...
    assert _pairs(assertions_from_posts("p1", [CLERK, MAYOR], [])) == [
        (AssertionKind.REJECT, MAYOR),
        (AssertionKind.REJECT, CLERK),
    ]
...
def test_the_claims_do_not_depend_on_the_order_they_were_given():
    ...
    forwards = assertions_from_posts("p1", [MAYOR, CLERK], [CLERK])
    backwards = assertions_from_posts("p1", [CLERK, MAYOR], [CLERK])
...
def test_every_claim_carries_a_source():
    ...
    claims = assertions_from_posts("p1", [MAYOR], [CLERK], changeset_id="c1")
