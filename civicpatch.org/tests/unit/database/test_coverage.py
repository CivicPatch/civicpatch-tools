...
async def test_get_municipality_rows_classifies_status_per_row():
    scraped_at = datetime.datetime(2026, 4, 1, tzinfo=datetime.timezone.utc)
    rows = [
        # (jurisdiction_ocdid, name, officials_count, scraped_at, has_url, is_fresh)
        ("zz-fresh", "Fresh City", 5, scraped_at, True, True),
        ("zz-stale", "Stale City", 3, scraped_at, True, False),
    ...
    assert result == [
        {
            ...
            "status": "fresh",
            "officials_count": 5,
            "last_verified_at": scraped_at.isoformat(),
        },
        {
