...
async def test_review_issue_pr_merged_resolves_issue():
    ...
    with (
        patch(
            "routers.webhooks.github.review_issues_db.get_review_issue_by_pull_request_url",
            new_callable=AsyncMock,
            return_value={"id": ISSUE_ID, "status": "pr_opened"},
        ),
        patch(
            "routers.webhooks.github.review_issues_db.resolve_review_issue",
            new_callable=AsyncMock,
        ) as mock_resolve,
        patch(
            "routers.webhooks.github.review_issues_db.reopen_review_issue",
            new_callable=AsyncMock,
        ) as mock_reopen,
    ):
