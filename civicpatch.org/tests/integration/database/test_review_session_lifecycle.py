...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_recent_activity_lists_this_users_own_activity_newest_first(test_user):
    """The review landing page's "recent activity" widget reads straight off
    activity, scoped to the current user — never another reviewer's rows, and
    ...
    try:
        await create_activity_row(ActivityType.EDIT_PERSON, str(test_user), ocdid, changeset_id)
        await create_activity_row(ActivityType.PUBLISH_REVIEW, str(test_user), ocdid, changeset_id)

        stats = await get_review_stats(str(test_user), _STATE_CODE)
        recent = stats["recent_activity"]

        assert len(recent) == 2
        assert recent[0]["type"] == ActivityType.PUBLISH_REVIEW.value, "newest entry first"
        assert recent[1]["type"] == ActivityType.EDIT_PERSON.value
        assert recent[0]["summary"], "summarize_activity always returns a non-empty string"
    finally:
        pool = await get_pool()
        async with pool.connection() as conn:
            await conn.execute("DELETE FROM activity WHERE user_id = %s", (test_user,))
        await _cleanup_open_pr(changeset_id, ocdid)
