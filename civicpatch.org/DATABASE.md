...
erDiagram
    ...
    changesets {
        ...
        text            state               "164: GENERATED — running|failed|ready|published|dismissed. The lifecycle, computed once instead of derived in ten predicates. Matches core/changeset_lifecycle.ChangesetState"
    }
...
- `jurisdictions.scraped_at` is "last _scraped_" — stamped on job-PR merge, **not** bumped by manual people edits (so hand-corrected jurisdictions don't read as freshly scraped)
