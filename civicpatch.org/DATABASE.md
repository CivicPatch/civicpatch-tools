...
erDiagram
    ...
    changesets {
        ...
        text_null       dismissed_reason    "CHECK rejected|cancelled|errored|superseded|expired (208: expired = a sheet import the reaper dismissed after a week undecided)"
        ...
        uuid_null       parent_changeset_id FK  "189. idx. ON DELETE SET NULL. The changeset this one layers on — live_roster_changeset (any kind except jurisdiction_edit) at mint time, resolved once rather than re-derived by every reader. A review pass's people_edit is the exception (223): its parent is the scrape it corrects. idx: UNIQUE (parent_changeset_id, created_by_user_id) WHERE kind = people_edit AND open — one review edit per person per scrape"
...
- **`changesets.organization_id` arrived in migration 158 and was dropped by 207.** A review is no longer about one body: the derivation files each person under the body that sighted them, and publishing closes only in the bodies the changeset read (`source_records.organization_id`). What 158 was for: A review is one organization at a time, and `posts_identity_uq` is `(organization_id, role_id, division_ocdid)` — the organization is the scope a post's identity sits inside. Until 158 that scope was implied: `posts.create_all` and `publications._bind_memberships` each resolved it with `organizations.find_or_create(jurisdiction)`, which is right only while a jurisdiction has exactly one body. Deliberately not on `DerivedPost`: a scrape reads a page and has no evidence for "this is the school board rather than the council" — which body is being scraped is a targeting decision, like `jurisdiction_ocdid` itself. Nullable, backfilled from each jurisdiction's single existing organization; the 16 rows left null are scrapes against jurisdictions that have never published, and `organizations.find_or_create_for_changeset` fills them in as they do.
- **`organizations` stopped being populated lazily in migration 195.** It backfilled a default `'Government'` organization for every jurisdiction that lacked one — 6,086 rows, taking the table from 3,438 to 9,524 — and `services/sources/open_data.py` now creates one the moment a jurisdiction is synced in (active or not), so the invariant "every jurisdiction has a default organization" holds unconditionally going forward rather than only for jurisdictions that happen to have published. Motivated by source-page caching needing to key off `organization_id` (jurisdictions are moving to multiple bodies), which has nowhere to attach for a jurisdiction that has never published. `database.organizations.find_or_create(jurisdiction)` — the lazy path this replaces for the default-name case — is now `get_default`, which looks up and raises rather than creates; `find_or_create` itself stays, for the still-unbuilt case of a named, non-default body.
