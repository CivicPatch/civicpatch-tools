...
def get_router(api_key_header):
    ...
    # Keyed on changeset_id, not a pull request number: a scrape published straight to open-data
    # has no pull request, and those are the majority now.
    @router.delete("/{changeset_id}", include_in_schema=False)
    ...
    @router.post("/{changeset_id}/save", include_in_schema=False)
