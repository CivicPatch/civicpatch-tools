...
const NOBODY_MERGED = new Map<string, string>();
...
describe("rosterEditPayload", () => {
  it("carries a person's changed fields through untouched", () => {
    expect(rosterEditPayload({
      patch: [patch("p1", { name: "Ann Lee" })],
      officeEdits: [],
      removedIds: [],
  ...
  it("turns an office pick into the whole set of posts that person holds", () => {
    const payload = rosterEditPayload({
      patch: [patch("p1", { name: "Ann Lee" })],
      officeEdits: [{ personId: "p1", postId: CLERK, membershipLabel: "Acting Clerk", organizationId: null }],
      removedIds: [],
      mergedInto: NOBODY_MERGED,
    });
  ...
  it("reads a brand-new person's pick out of their fields", () => {
    ...
    const payload = rosterEditPayload({
      patch: [patch("new-1", { name: "Bo Nguyen", post_id: MAYOR, membership_label: "Mayor" })],
      officeEdits: [],
      removedIds: [],
      mergedInto: NOBODY_MERGED,
    });
  ...
  it("never sends the office fields as person fields", () => {
    const [person] = rosterEditPayload({
      patch: [patch("p1", { post_id: MAYOR, membership_label: "x" })],
      officeEdits: [],
      removedIds: [],
      mergedInto: NOBODY_MERGED,
    });
  ...
  it("lets a removal take the office while their other edits stand", () => {
    ...
    const payload = rosterEditPayload({
      patch: [patch("p1", { name: "Ann Lee" })],
      officeEdits: [{ personId: "p1", postId: CLERK, membershipLabel: null, organizationId: null }],
      removedIds: ["p1"],
      mergedInto: NOBODY_MERGED,
    });
  ...
  it("sends one entry per person, whatever they appear in", () => {
    const payload = rosterEditPayload({
      patch: [patch("p1", { name: "Ann Lee" }), patch("p2", { name: "Bo Nguyen" })],
      officeEdits: [{ personId: "p2", postId: MAYOR, membershipLabel: null, organizationId: null }],
      removedIds: ["p3"],
      mergedInto: NOBODY_MERGED,
    });
  ...
  it("removing every row leaves them holding nothing", () => {
    ...
    const payload = rosterEditPayload({
      patch: [],
      officeEdits: [],
      removedIds: ["p1:org-council", "p1:org-schools"],
      mergedInto: NOBODY_MERGED,
      heldOffices: held,
    });
  ...
  it("sends a merged person as same_as and nothing else", () => {
    // Claims filed against the absorbed id land on the survivor, so a removal riding along
    // would take the survivor off the roster.
    const payload = rosterEditPayload({
      patch: [patch("p1", { name: "Ann Lee" })],
      officeEdits: [],
      removedIds: ["p2"],
      mergedInto: new Map([["p2", "p1"]]),
    });
