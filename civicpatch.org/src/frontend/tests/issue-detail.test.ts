...
describe("getIssueDetail", () => {
  it("shows the merge error string for a merge_failed issue", () => {
    ...
    expect(getIssueDetail(ISSUE_TYPE.MERGE_FAILED, REQUEST_ID, data)).toBe("Pull request is blocked");
  });

  it("falls back to the issue key when a merge_failed issue has no error", () => {
    const data = { mergeable_state: null };
    expect(getIssueDetail(ISSUE_TYPE.MERGE_FAILED, REQUEST_ID, data)).toBe(REQUEST_ID);
  });

  it("falls back to the issue key when a merge_failed issue has no data", () => {
    expect(getIssueDetail(ISSUE_TYPE.MERGE_FAILED, REQUEST_ID, null)).toBe(REQUEST_ID);
  });

  it("does not surface a stored error for non-merge_failed types", () => {
    const data = { error: "some pipeline error" };
    expect(getIssueDetail(ISSUE_TYPE.PIPELINE_ERROR, REQUEST_ID, data)).toBe(REQUEST_ID);
  });
