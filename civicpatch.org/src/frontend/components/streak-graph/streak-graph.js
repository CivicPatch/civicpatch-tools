    ...
    <div class="streak-graph" @click=${handleCopy}>
      ...
      <div class="streak-graph__body">
        <div class="streak-graph__dow-col">
          ${DOW_LABELS.map(
            (d) => html`<span class="streak-graph__dow">${d}</span>`,
          )}
        </div>
        <div class="streak-graph__right">
          <div class="streak-graph__months">
            ${monthLabels.map(
              (label) =>
                html`<span class="streak-graph__month">${label}</span>`,
            )}
          </div>
          <div class="streak-graph__grid">
            ${days.map(
              ({ date, count, isFuture, isToday }) => html`
                <div
                  class="streak-graph__day ${count > 0
                    ? "streak-graph__day--active"
                    : isFuture
                      ? "streak-graph__day--future"
                      : "streak-graph__day--empty"}${isToday
                    ? " streak-graph__day--today"
                    : ""}"
                  style="${count > 0
                    ? `--day-opacity:${getOpacity(count).toFixed(2)}`
                    : ""}"
                  title="${!isFuture
                    ? count
      ...
      <i
        class="streak-graph__copy-icon ${copied
          ? "streak-graph__copy-icon--copied fa-solid fa-check"
          : "fa-regular fa-copy"}"
      ></i>
    </div>
  `;
...
customElements.define(
  "civ-streak-graph",
  component(StreakGraph, { useShadowDOM: false }),
);
