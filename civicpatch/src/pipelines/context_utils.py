...
def load_context_from_file(jurisdiction_id: str) -> Optional[PipelineContext]:
    ...
    try:
        with open(context_file_path, "r") as f:
            ...
            return PipelineContext.model_validate(context_data)
    except Exception as e:
        print(f"Warning: Could not load context: {e}")
        return None

async def save_context_to_file(context: PipelineContext) -> None:
...
def _apply_config_priority(context: PipelineContext, jurisdiction_id: str, pipeline_request: PipelineRequest) -> PipelineContext:
...
def create_pipeline_context(request_id: str, pipeline_request: PipelineRequest) -> PipelineContext:
...
def load_or_create_context(request_id: str, pipeline_request: PipelineRequest) -> PipelineContext:
    jurisdiction_id = pipeline_request.jurisdiction_id
