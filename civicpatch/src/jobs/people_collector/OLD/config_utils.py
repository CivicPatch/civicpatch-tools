...
def save_config_to_file(jurisdiction_id: str, config_updates: WorkflowConfig) -> None:
    config_file_path = data_path_utils.get_config_file_path(jurisdiction_id)
    os.makedirs(os.path.dirname(config_file_path), exist_ok=True)
    
    existing_config = load_config_from_file(jurisdiction_id)
    
    # Convert WorkflowConfig to dict before merging
...
def merge_config_into_context(context: PeopleCollectorContext, config: Dict[str, Any]) -> PeopleCollectorContext:
    context_dict = context.model_dump()
    
    # Only merge config fields that exist in the PeopleCollectorContext model
    valid_fields = set(PeopleCollectorContext.model_fields.keys())
    filtered_config = {k: v for k, v in config.items() if k in valid_fields}
    
    context_dict.update(filtered_config)
    
    return PeopleCollectorContext.model_validate(context_dict)