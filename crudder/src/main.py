import os
import datetime
from fastapi import FastAPI, Request, Security, HTTPException, Depends, Form
...
import sqlite3
from database import maybe_init_db, maybe_insert_user, create_api_key, get_api_keys_for_user, revoke_api_key
...
if not all([APPROVED_GITHUB_PROVIDER_USER_IDS, MAINTAINER_EMAIL, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, APP_ENVIRONMENT, GITHUB_WORKFLOW_TOKEN, JWT_SECRET_KEY]):
...
def trigger_github_data_intake_workflow(server_name: str, zip_file_url: str):
...
async def post_api_keys(request: Request, user: OpenID = Depends(get_logged_user)):
    ...
    provider_user_id = user.id
    api_key = create_api_key(db_connection, db_cursor, provider, provider_user_id)
...
async def login_callback(request: Request, provider: str):
    ...
    maybe_insert_user(db_connection, db_cursor, "github", openid.id)
...
@app.post("/update_server", include_in_schema=False)
async def update_server(
    ...
    # Update the user's server_name and server_url in the database
    db_cursor.execute(
        "UPDATE users SET server_name = ?, server_url = ? WHERE provider = ? AND provider_user_id = ?",
        (server_name, server_url, user.provider, user.id)
    )
