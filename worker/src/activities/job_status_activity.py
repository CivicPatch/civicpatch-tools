...
@activity.defn
async def update_job_status(changeset_id: str, status: str, progress: Optional[int] = None) -> None:
...
@activity.defn
async def poll_job_status(changeset_id: str) -> str:
    while True:
        activity.heartbeat(f"polling job {changeset_id}")
        status = None
        try:
            async with httpx.AsyncClient(headers=_HEADERS, timeout=15) as client:
            ...
            activity.logger.warning(f"Job {changeset_id}: server error {e.response.status_code}, will retry")
        except httpx.HTTPError as e:
            activity.logger.warning(f"Job {changeset_id}: transient error ({type(e).__name__}), will retry")
        if status is not None:
            activity.logger.info(f"Job {changeset_id}: status={status}")
