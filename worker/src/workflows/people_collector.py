...
with workflow.unsafe.imports_passed_through():
    from activities.github_activity import trigger_github_action, poll_run_status
...
class PeopleCollectorWorkflow:
    ...
    async def run(self, jurisdiction_ocdid: str, request_id: str) -> str:
        ...
        if conclusion == RunConclusion.FAILURE:
            await workflow.execute_activity(
                update_job_status,
                args=[request_id, JobStatus.PAUSED],
                start_to_close_timeout=timedelta(seconds=30),
            )
            workflow.logger.info(f"Run {run_id} paused — waiting for human_approval signal")
