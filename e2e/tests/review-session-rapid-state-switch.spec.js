 ...
 *
 * Repro: NJ (3 PRs) → advance a couple cards → look at TX (1 PR) → start TX → back to NJ →
 * Resume. The visible card always matches the URL's state — which is now literally true: the
 * navbar picker is gone and state travels in `?state=`.
 */
...
test.describe("Review session — rapid state switch", () => {
  test("switching state from within a session never shows the previous state's card", async ({ authenticatedPage: page }) => {
    ...
    // Look at TX — a full nav to the TX landing. No NJ card.
    ...
    await expect(page).toHaveURL(/\/review(\?|$)/);
    await expect(
      page.locator(".review-session__jurisdiction").filter({ hasText: "E2E TX City" }),
    ).toHaveCount(0);
