...
test.describe("Review session — Next button disabled", () => {
  test("Next button is disabled when only one PR is available in the session", async ({ authenticatedPage: page }) => {
    // Switch to TX (seed has exactly 1 open PR there). This used to pick the state from a
    // `.nav-state-selector` in the nav; that control is gone, and the landing page reads the
    // state from its own `?state=` parameter (`review-routes.ts`'s `landingUrl`), falling back
    ...
    await expect(page.locator(".review-session__next-btn")).toBeDisabled();
  });
});
