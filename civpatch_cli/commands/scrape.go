...
func ScrapeRun(ctx context.Context, state string, geoid string, createPr bool, develop bool, withCi bool, sendCosts bool, branchName string, githubEnv string, prNumber int) error {
	...
	cmd := []string{
		...
		"&&",
		fmt.Sprintf("cd /app/civpatch && xvfb-run rake 'pipeline:scrape[%s,%s,%t]'", state, geoid, createPr),
	}
	...
	if createPr {
		cmd = append(cmd, "&&", fmt.Sprintf("/scripts/create_pull_request.sh %s %s", state, geoid))
	} else {
		cmd = append(cmd, "&&", fmt.Sprintf("/scripts/update_branch.sh %s %s", state, geoid))
