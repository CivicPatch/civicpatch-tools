...
async def send_success(context: PeopleCollectorContext, api_client: httpx.AsyncClient) -> MaybeSendToGitHubStep:
    ...
    zip_file_path = file_utils.zip_job_artifacts(pipeline_run_id, jurisdiction_ocdid, include_data=True)
    ...
    response = await services.civicpatch_api.submit_job_artifacts(
        api_client, pipeline_run_id, jurisdiction_ocdid, zip_file_path, pipeline_run_status=PipelineRunStatus.SUCCESS
    )
    if not response:
