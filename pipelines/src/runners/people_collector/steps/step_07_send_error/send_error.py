...
async def send_error(context: PeopleCollectorContext, api_client: httpx.AsyncClient) -> MaybeSendToGitHubStep:
    ...
    try:
        zip_file_path = file_utils.zip_job_artifacts(
            context.pipeline_run_id, context.data.jurisdiction_ocdid, include_data=False
        )

        response = await services.civicpatch_api.submit_job_artifacts(
            api_client, context.pipeline_run_id, context.data.jurisdiction_ocdid, zip_file_path, pipeline_run_status=PipelineRunStatus.ERROR
        )
        if not response:
