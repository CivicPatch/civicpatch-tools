...
import pipelines_environment


def _make_test_env():
    env = {}
    for var in pipelines_environment.REQUIRED_ENV_VARS + pipelines_environment.OPTIONAL_ENV_VARS:
...
def patch_get_env_vars(request, monkeypatch):
    if request.node.get_closest_marker("evals") or request.node.get_closest_marker("evals_relevant"):
        yield
        return
    monkeypatch.setattr("pipelines_environment.get_env_vars", _make_test_env)
    yield
