...
def test_known_roles_from_db_extracts_office_names():
    ...
    assert _known_roles_from_db(existing, _ROLE_CONFIG) == ["Mayor", "Council Member"]
...
def test_known_roles_from_db_deduplicates():
    ...
    assert _known_roles_from_db(existing, _ROLE_CONFIG) == ["Council Member"]
...
def test_known_roles_from_db_handles_missing_office():
    ...
    assert _known_roles_from_db(existing, _ROLE_CONFIG) == ["Mayor"]


def test_known_roles_from_db_empty():
    assert _known_roles_from_db([]) == []
...
def test_known_roles_from_db_splits_compound_office_names():
    ...
    result = _known_roles_from_db(existing, _ROLE_CONFIG)
